package bartish.crypto;

import sun.security.util.BitArray;

import java.nio.charset.StandardCharsets;
import java.util.Random;
import java.util.Scanner;

public class Main {
    private static final int[] replaceTable = {
            57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7, 56, 48, 40, 32, 24, 16, 8, 0, 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6
    };


    private static final int[] extensionTable = {
            31, 0, 1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 7, 8, 9, 10, 11, 12, 11, 12, 13, 14, 15, 16, 15, 16, 17, 18, 19, 20, 19, 20, 21, 22, 23, 24, 23, 24, 25, 26, 27, 28, 27, 28, 29, 30, 31, 0
    };


    private static final int[][][] blockTransformationTables = {
            {
                    {14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7},
                    {0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8},
                    {4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0},
                    {15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13}
            },
            {
                    {15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10},
                    {3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5},
                    {0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15},
                    {13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9}
            },
            {
                    {10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8},
                    {13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1},
                    {13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7},
                    {1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12}
            },
            {
                    {7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15},
                    {13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9},
                    {10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4},
                    {3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14}
            },
            {
                    {2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9},
                    {14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6},
                    {4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14},
                    {11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3}
            },
            {
                    {12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11},
                    {10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8},
                    {9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6},
                    {4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13}
            },
            {
                    {4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1},
                    {13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6},
                    {1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2},
                    {6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12}
            },
            {
                    {13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7},
                    {1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2},
                    {7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8},
                    {2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11}
            }
    };


    private static final int[] replaceTableP = {
            15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25, 4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10, 3, 24
    };


    private static final int[] replaceTableC = {
            56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35
    };


    private static final int[] replaceTableD = {
            62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3
    };


    private static final int[] shiftTable = {
            1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1
    };


    private static final int[] keyCreateTable = {
            13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40, 51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31
    };


    public static void main(String[] args) {
        byte[] array = new byte[8];
        new Random().nextBytes(array);
        String vectorInit = new String(array, StandardCharsets.UTF_8);

        Scanner scanner = new Scanner(System.in);
        System.out.println("password (length must be 7):");
        String password = scanner.nextLine();

        System.out.println("text (length bounded by 8):");
        String start = scanner.nextLine();

        String str = encrypt(start, password, vectorInit);
        System.out.println("encrypt: " + str);
        str = decrypt(str, password, vectorInit);
        System.out.println("decrypt: " + str);
    }


    static class Key {
        private BitArray newKey = new BitArray(64);
        private BitArray[] keys = new BitArray[16];

        public Key(
                String keyS
        ) {
            if (keyS.length() != 7) throw new IllegalArgumentException("Key not equal 7");

            BitArray key = new BitArray(56, keyS.getBytes());
            for (int i = 0; i < 8; i++) {
                boolean b1 = true;
                for (int j = 0; j < 7; j++) {
                    newKey.set(i * 8 + j, key.get(i * 7 + j));
                    b1 = b1 ^ key.get(i * 7 + j);
                }
                newKey.set(i * 7 + 7, b1);
            }
            initKeys();
        }

        private void shift(
                BitArray bitArray,
                int a
        ) {
            while (a != 0) {
                a--;
                boolean b = bitArray.get(0);
                for (int i = 0; i < 27; i++) {
                    bitArray.set(i, bitArray.get(i + 1));
                }
                bitArray.set(27, b);
            }
        }


        private void initKeys() {
            BitArray C = new BitArray(28), D = new BitArray(28);

            for (int i = 0; i < 28; i++) {
                C.set(i, newKey.get(replaceTableC[i]));
                D.set(i, newKey.get(replaceTableD[i]));
            }

            for (int i = 0; i < 16; i++) {
                shift(C, shiftTable[i]);
                shift(D, shiftTable[i]);
                keys[i] = new BitArray(48);
                for (int j = 0; j < 48; j++) {
                    if (keyCreateTable[i] < 28)
                        keys[i].set(i, C.get(keyCreateTable[i]));
                    else
                        keys[i].set(i, D.get(keyCreateTable[i] - 28));
                }
            }

        }

        public BitArray getKey(
                int i
        ) {
            return keys[i];
        }

    }


    private static BitArray[] splitBlock(
            BitArray E
    ) {
        BitArray[] blocks = new BitArray[8];

        for (int i = 0; i < 8; i++) {
            blocks[i] = new BitArray(6);
            for (int j = 0; j < 6; j++) {
                blocks[i].set(j, E.get(i * 6 + j));
            }
        }
        return blocks;
    }


    private static BitArray extendBlock(
            BitArray R
    ) {
        BitArray E = new BitArray(48);
        for (int i = 0; i < 48; i++) {
            E.set(i, R.get(extensionTable[i]));
        }
        return E;
    }


    private static BitArray xor(
            BitArray a,
            BitArray b
    ) {
        if (a.length() != b.length()) throw new IllegalArgumentException("input not bounded 8");

        BitArray result = new BitArray(b.length());
        for (int i = 0; i < a.length(); i++) {
            result.set(i, a.get(i) ^ b.get(i));
        }
        return result;
    }


    private static BitArray helper (
            BitArray R,
            BitArray key
    ) {
        BitArray result = new BitArray(32);

        BitArray extendBlock = extendBlock(R);
        extendBlock = xor(extendBlock, key);


        BitArray[] blocks = splitBlock(extendBlock);
        BitArray tempRes = new BitArray(32);
        for (int i = 0; i < 8; i++) {
            int a = toInt(blocks[i].get(0)) * 2 + toInt(blocks[i].get(5));
            int b = toInt(blocks[i].get(1)) * 8 + toInt(blocks[i].get(2)) * 4 + toInt(blocks[i].get(3)) * 2 + toInt(blocks[i].get(4));
            int c = blockTransformationTables[i][a][b];
            for (int j = 3; j >= 0; j--) {
                tempRes.set(i * 4 + j, (c % 2 == 1));
                c = c / 2;
            }

        }
        for (int i = 0; i < 32; i++) {
            result.set(i, tempRes.get(replaceTableP[i]));
        }
        return result;
    }


    private static BitArray feistelEncrypt(
            BitArray input,
            Key key
    ) {
        BitArray output = new BitArray(64),
                L = new BitArray(32),
                R = new BitArray(32),
                temp, f;
        for (int i = 0; i < 32; i++) {
            L.set(i, input.get(i));
            R.set(i, input.get(i + 32));
        }

        for (int i = 0; i < 16; i++) {
            BitArray subKey = key.getKey(i);
            temp = R;
            f = helper(R, subKey);
            R = xor(L, f);
            L = temp;
        }

        for (int i = 0; i < 32; i++) {
            output.set(i, L.get(i));
            output.set(i + 32, R.get(i));
        }

        return output;
    }

    private static BitArray feistelDecrypt(
            BitArray b,
            Key key
    ) {
        BitArray output = new BitArray(64),
                L = new BitArray(32),
                R = new BitArray(32),
                temp, f;
        for (int i = 0; i < 32; i++) {
            L.set(i, b.get(i));
            R.set(i, b.get(i + 32));
        }

        for (int i = 0; i < 16; i++) {
            BitArray subKey = key.getKey(15 - i);
            temp = L;
            f = helper(L, subKey);
            L = xor(R, f);
            R = temp;
        }

        for (int i = 0; i < 32; i++) {
            output.set(i, L.get(i));
            output.set(i + 32, R.get(i));
        }
        return output;
    }

    private static BitArray sReplace (
            BitArray input
    ) {
        BitArray output = new BitArray(input.length());
        for (int i = 0; i < 64; i++) {
            output.set(i, input.get(replaceTable[i]));
        }
        return output;
    }

    private static BitArray bReplace (
            BitArray input
    ) {
        BitArray output = new BitArray(input.length());
        for (int i = 0; i < 64; i++) {
            output.set(replaceTable[i], input.get(i));
        }
        return output;
    }

    private static String encrypt(
            String input,
            String keyS,
            String initStr
    ) {
        Key key = new Key(keyS);

        while (input.length() % 8 != 0)
            input = input.concat(String.valueOf('\0'));
        String output = "";

        BitArray previousEncryptedBlock = new BitArray(64, initStr.getBytes());


        for (int i = 0; i < input.length() / 8; i++) {
            BitArray arr = new BitArray(64, input.substring(i * 8, (i + 1) * 8).getBytes());

            arr = xor(previousEncryptedBlock, arr);

            arr = sReplace(arr);
            arr = feistelEncrypt(arr, key);
            arr = bReplace(arr);

            previousEncryptedBlock = arr;
            output = output.concat(new String(arr.toByteArray()));
        }

        return output;
    }

    private static String decrypt(
            String input,
            String keyS,
            String initStr
    ) {
        if (input.length() % 8 != 0)
            throw new IllegalArgumentException("input has incorrect size");

        Key key = new Key(keyS);
        String result = "";

        BitArray previousEncryptedBlock = new BitArray(64, initStr.getBytes());

        for (int i = 0; i < input.length() / 8; i++) {
            if (i != 0)
                previousEncryptedBlock = new BitArray(64, input.substring((i - 1) * 8, (i) * 8).getBytes());

            BitArray arr = new BitArray(64, input.substring(i * 8, (i + 1) * 8).getBytes());

            arr = sReplace(arr);
            arr = feistelDecrypt(arr, key);
            arr = bReplace(arr);

            arr = xor(previousEncryptedBlock, arr);
            result = result.concat(new String(arr.toByteArray()));
        }

        return result;
    }

    public static int toInt(Boolean b) {
        return b ? 1 : 0;
    }
}
